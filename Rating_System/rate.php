...
if(!$voted) {     //if the user hasn't yet voted, then vote normally...
    ...
    if (($vote_sent >= 1 && $vote_sent <= $units))  // keep votes within range, make sure IP matches
    {
        ...
        $updateQuery = "UPDATE $rating_tableName SET total_votes='".$added."', total_value='".$sum."', used_ips='".$insertip."' WHERE rating_id='$id_sent'";
...
$new = $db->prepare("SELECT total_votes, total_value, used_ips FROM $rating_tableName WHERE rating_id='$id_sent' ")or die(" Error: ");
//$numbers = mysql_fetch_assoc($newtotals);
$newtotals = $new->execute();
$numbers = $new->fetch(PDO::FETCH_ASSOC);
//$numbers = mysql_fetch_assoc($newtotals);
$count = $numbers['total_votes'];  //how many votes total
$current_rating = $numbers['total_value'];  //total number of rating added together and stored
$tense = ($count==1) ? "vote" : "votes";  //plural form votes/vote
